...
With GitHub in order to keep your OTTR course preserved content and code is managed through the use of branches (basically just tracked copies of your whole repository). To explain branches we'll mainly refer to two branches: your `main` branch:
...
It's ideal to work on your course from your computer (you do not need to install any other software besides Git if you don't want to). To get going, you will need to `clone` the course repository to your own computer. Cloning is just making a local copy on your computer of the remote version of the project on GitHub.
...
Handling branches is where you unleash the real benefit of GitHub, but it can be confusing and a bit tricky to get a hang of.
...
Now we can edit our files and code just like we normally would. Go ahead and make an edit to [some of the files listed in the above section](#file-changes-to-be-made).
...
Now that we have changes committed to our branch we are ready to also add them to the remote, GitHub internet copy! To do this, we will need to `push` our branch.
