...
If you have a personal repository, or a organization that hasn't used github actions before, you'll also need to check that workflow actions have been enabled. Go to the `Settings` and then `Actions` Make sure that you've given read and write permissions and have also checked `Allow GitHub Actions to create and approve pull requests`. 
...
- For `Secret`: Create a personal access token [following these instructions](https://docs.github.com/en/github/authenticating-to-github/creating-a-personal-access-token#creating-a-token). Underneath `Select scopes`, check both `repo` and `workflow`.
Then copy the PAT and save as the secret. Your GitHub actions should all be able to run smoothly now.
